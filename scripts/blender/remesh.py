...
def ensure_active_mesh(obj):
    """Convert the active Curve in place and return the resulting Mesh object."""
    ...
    if obj.type != 'CURVE':
        raise RuntimeError(
            f"当前对象类型为 {obj.type}，只支持 Mesh 或 Curve。"
        )

    if obj.mode != 'OBJECT':
        bpy.ops.object.mode_set(mode='OBJECT')

    # Object Convert acts on every selected convertible object. Isolate the
    # active pattern so unrelated selected curves are not modified.
    ...
    if result != {'FINISHED'}:
        raise RuntimeError(
            f"Curve '{source_name}' 自动转换 Mesh 失败。"
        )
    ...
    if converted is None or converted.type != 'MESH':
        raise RuntimeError(
            f"Curve '{source_name}' 转换后没有得到活动 Mesh。"
        )

    print(
        f"Curve 自动转换完成: {source_name} → {converted.name} "
        f"({len(converted.data.vertices)} vertices)"
    )
...
boundary_index_loops = find_boundary_loops(mesh)

boundary_3d_loops = [
    [mesh.vertices[index].co.copy() for index in loop]
    for loop in boundary_index_loops
]

all_boundary_3d = [
    point
    for loop in boundary_3d_loops
    for point in loop
]
...
normal = newell_normal(boundary_3d_loops[0])

origin = boundary_3d_loops[0][0].copy()
...
for boundary_3d in boundary_3d_loops:
...
boundary_2d_raw_loops = [
    [project_2d(point) for point in boundary_3d]
    for boundary_3d in boundary_3d_loops
]
...
# 保证每块独立版片 CCW
for boundary_2d_raw in boundary_2d_raw_loops:
...
boundary_loops = [
    resample_closed_boundary(boundary_2d_raw, TARGET)
    for boundary_2d_raw in boundary_2d_raw_loops
]
...
boundary = [point for loop in boundary_loops for point in loop]
...
for panel_boundary in boundary_loops:

    min_x = min(p.x for p in panel_boundary)
    max_x = max(p.x for p in panel_boundary)
    min_y = min(p.y for p in panel_boundary)
    max_y = max(p.y for p in panel_boundary)
...
for panel_boundary in boundary_loops:
    ...
    panel_count = len(panel_boundary)
    panel_indices = list(range(boundary_offset, boundary_offset + panel_count))

    constraint_edges.extend(
        (
            panel_indices[index],
            panel_indices[(index + 1) % panel_count],
        )
        for index in range(panel_count)
    )
    constraint_faces.append(panel_indices)
    boundary_offset += panel_count
...
new_obj["cloth_panel_count"] = len(boundary_loops)
