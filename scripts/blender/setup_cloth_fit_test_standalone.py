...
# Procedural seam preview.  generate_sewing_standalone.py creates semantic
...
def _write_seam_mask(obj):
    ...
    if not group_indices:
        raise RuntimeError(
            f"{obj.name} 没有 SEAM 顶点组；请先运行 generate_sewing_standalone.py。"
        )
...
def _validate_sewing(obj):
    ...
    if ENABLE_SEWING and not loose:
        raise RuntimeError(
            f"{obj.name} 没有 Loose Edge；请先运行 generate_sewing_standalone.py。"
        )
