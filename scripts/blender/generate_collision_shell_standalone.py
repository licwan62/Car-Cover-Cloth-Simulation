...
55 mm solidified copy. Stage two extracts a coarser collision surface only from
that shell, smooths and decimates it, restores source bounds, validates manifold
topology, and enables Collision physics.
"""
...
# engine below it. Probe the upper silhouette before accepting stage one. Only
# a damaged result is rebuilt; the temporary Solidify never touches the source.
ENABLE_THIN_SURFACE_REPAIR = True
THIN_SURFACE_PROTECTION_MM = 55.0
...
def apply_thin_surface_protection(obj):
    """Give open panels temporary volume so voxel remesh cannot discard them."""
